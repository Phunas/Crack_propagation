...
def find_tip_broken_bonds(atoms, cutoff, bulk_nn=4, boundary_thickness=None):
    ...
    for m in range(0, len(broken_bonds_array[0])):
        ...
        if temp_atom_pos[0] > atoms.positions[first_atom_number,0]:
    ...
    method = 2
    if method == 1:
        for n in range(0, len(broken_bonds_array[0][n]])):
            if n == m:
                pass
            else:
                temp_atom_pos_2 = atoms.positions[broken_bonds_array[0][n]]
                if temp_atom_pos_2[0] > atoms.positions[first_atom_number,0]:
                    if left_boundary < tmp_atom_pos_2[0] < right_boundary:
                        if bottom_boudary < temp_atom_pos_2[1] < top_boundary:
                            second_atom_number = n

    if method == 2: 
        i, d = neighbour_list("id", atoms[broken_bonds_array[0][first_atom_number]], cutoff+1.0)
        indecies_and_distances = zip(i,d)
        sorted_indecies_and_distances = sorted(indecies_and_distances_array, key = item[1])
        second = sorted_indecies_and_distances[3,0]
            
    tip_position = atoms.positions[broken_bonds_array[0][first_atom_number]]/2 + atoms.positions[broken_bonds_array[0][second_atom_number]]
